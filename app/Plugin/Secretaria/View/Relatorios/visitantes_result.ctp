<?php
	...
	$header = array(utf8_decode('Nome'), utf8_decode('E-mail'), utf8_decode('Telefone'), utf8_decode('Celular'), utf8_decode('CPF'), utf8_decode('Cadastrado em'));
	...
	foreach ($membros as $key => $value) {
		...
		$data = explode(' ', $value['Membro']['created']);
		$data = explode('-', $data[0]);
		$data = array_reverse($data);
		$data = $data[0].'/'.$data[1].'/'.$data[2];
		
		$pdf->SetFont('Helvetica', '', 8);
		$pdf->Row(array(utf8_decode($value['Membro']['nome']), utf8_decode($value['Membro']['email']), utf8_decode($value['Membro']['fone']), utf8_decode($value['Membro']['cel']), utf8_decode($value['Membro']['cpf']), utf8_decode($data)));
	}
	$pdf->Output();
?>