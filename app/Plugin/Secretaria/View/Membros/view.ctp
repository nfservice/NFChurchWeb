...
<div id="parente">
  <?php
    ...
    foreach ($this->request->data['Relacionamento'] as $relacionamento) {
      echo $this->Form->input('Relacionamento.'.$i.'.membro2_id', array('label' => 'Parente', 'class' => 'form-control', 'placeholder' => 'Nome Parente', 'required', 'div' => array('class' => 'form-group col-md-8'), 'options' => $parentes, 'default' => $relacionamento['membro2_id'], 'disabled'));
      echo $this->Form->input('Relacionamento.'.$i.'.tiporelacionamento_id', array('label' => 'Tipo:', 'class' => 'form-control', 'required', 'div' => array('class' => 'form-group col-md-2'), 'options' => $relacionamentos, 'default' => $relacionamento['tiporelacionamento_id'], 'disabled'));
      $i++;     
    }
  ?>
</div>
