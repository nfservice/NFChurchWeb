...
{	
	/*if ($this->PageNo() !== 1) {
	    ...
	    $this->Cell(30,5,utf8_decode('Código'), 'B', 0, 'C');
	    $this->Cell(64,5,utf8_decode('Descrição'), 'B', 0, 'C');
	    $this->Cell(24,5,utf8_decode('Quantidade'), 'B', 0, 'C');
	    $this->Cell(24,5,utf8_decode('Estoque Disp.'), 'B', 0, 'C');
	    $this->Cell(24,5,utf8_decode('Estoque Min.'), 'B', 0, 'C');
	    $this->Cell(24,5,utf8_decode('Estoque Min.'), 'B', 0, 'C');
	    // Quebra de linha
	    $this->Ln();
	}*/
...
function Footer()
{
    ...
    $this->Cell(0,5,'Pagina '.$this->PageNo().' - '.date('d/m/Y H:i:s'),0,0,'C');
}
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $pulaLinha = 2)
{
	...
	if($border && strpos($border,'B')!==false)
		$b .= 'B';
	//$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
	$this->Cell($w,$h,substr($s,$j,$i-$j),$b,$pulaLinha,$align,$fill);
	$this->x = $this->lMargin;
}
...
public function __textBox($x,$y,$w,$h,$text='',$aFont=array('font'=>'Times','size'=>8,'style'=>''),$vAlign='T',$hAlign='L',$border=1,$link='',$force=TRUE,$hmax=0,$vOffSet=0){       
    $oldY = $y;
    $temObs = FALSE;
    $resetou = FALSE;
    if ($w < 0 ) {
    ...
    if ( $border ) {
        $this->RoundedRect($x,$y,$w+5,$h,0.8,'5555','D');
    }
    //estabelecer o fonte
